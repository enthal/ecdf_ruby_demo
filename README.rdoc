...
Unit tests, included, helped me get each small part right.  I chose not to cover the higher-level operation, preferring manual testing in this case.  In a work environment, I might create unit or functional tests with canned data and known correct output, but I lacked that with this challenge problem.  Instead, this solution includes a data generator that produces payment data with per-user +is_card_present+ probability occurring in a Gaussian distribution, such that the output ECSFs under correct operation occur as a recognizable S-curve (especially when graphed).  In real work, tests could (I imagine) be constructed to mathematically show output ECDF fits input Gaussian data, by, e.g., standard deviation.  These could be maintained as part of a functional test suite.
...
FWIW, a complete procedural/functional implementation of the core algorithm was done at git commit XXXX in file +cardp.rb+.  I deemed it hard to understand for my expected audience and opted to bring in some OO, which I leave in the delivered solution.
...
* <tt>percentiles=</tt>_number_ - default:100; Group users into more or fewer intervals than the default 100.  Useful at small values while experimenting with, e.g., the Gaussian parameters, or parallelization timings.
